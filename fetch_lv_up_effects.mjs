...
import { PARSE_LV_UP_EFFECTS_JS } from './wiki_lv_up_parse.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const OUT = path.join(__dirname, 'lv_up_effects.json');
const BASE = 'https://w.atwiki.jp/bulletaction/pages';

export async function fetchLvUpEffects(options = {}) {
  ...
  for (const name of names) {
    ...
    for (let attempt = 1; attempt <= (options.retries ?? 3); attempt++) {
      ...
      try {
        ...
        const result = await page.evaluate(PARSE_LV_UP_EFFECTS_JS);
        if (result?.error) throw new Error(result.error);
        characters[name] = {
          pageId,
          url,
          moveCount: Object.keys(result.moves).length,
          moves: result.moves,
        };
        console.log(JSON.stringify({ character: name, pageId, moveCount: characters[name].moveCount, attempt }));
        lastErr = null;
        break;
      } catch (err) {
        lastErr = err;
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  const data = await fetchLvUpEffects({
    delayMs: 3000,

...
assert.equal(kurin['状態'].length, 8);
assert.equal(hou['状態'].length, 8);
assert.equal(kurin['状態'][0]['技名'], '近A');
assert.equal(kurin['状態'][0]['有利差']['正G'], '+9');
assert.equal(hou['状態'][0]['技名'], '6A');
assert.equal(hou['状態'][7]['技名'], 'DC');
assert.equal(hou['状態'][7]['有利差']['正G'], '-1');

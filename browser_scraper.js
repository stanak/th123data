// Browser-side scraper - run via CDP Runtime.evaluate
async function scrapeAllFrameData() {
  const CHARACTERS = [
    { name: '早苗', pageId: 157 },
    { name: 'チルノ', pageId: 159 },
    { name: '美鈴', pageId: 164 },
  ...
  function parseTable(table) {
    ...
    for (let r = headerRowCount; r < grid.length; r++) {
      ...
      row.forEach((cell, i) => {
        const val = cell || '';
        if (val) { has = true; setNested(obj, headers[i], val); carry[i] = val; }
        else if (carry[i]) setNested(obj, headers[i], carry[i]);
      });
      if (!has) continue;
      const prev = rows[rows.length - 1];
      if (prev && !obj['技名'] && Object.keys(obj).some(k => k !== '備考' && !k.startsWith('_col'))) {
        merge(prev, obj);
      } else rows.push(obj);
    }
    return { headers, rows };
      ...
      while (sib && !['H2','H3'].includes(sib.tagName)) {
        sib.querySelectorAll('a[href*="/pages/"]').forEach(a => {
          ...
          if (text && !text.startsWith('*') && href) {
            result.subpageLinks.push({ text, url: href.startsWith('http') ? href : 'https://w.atwiki.jp' + href });
          }
        });
  ...
  for (const char of CHARACTERS) {
    ...
    const charData = { name: char.name, pageUrl: url, title: parsed.title, frameData: {}, footnotes: parsed.footnotes };

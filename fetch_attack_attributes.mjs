...
import { PARSE_ATTACK_ATTRIBUTES_JS } from './wiki_attack_attr_parse.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const OUT = path.join(__dirname, 'attack_attributes.json');
const BASE = 'https://w.atwiki.jp/bulletaction/pages';

export async function fetchAttackAttributes(options = {}) {
  ...
  for (const name of names) {
    ...
    for (let attempt = 1; attempt <= (options.retries ?? 3); attempt++) {
      ...
      try {
        ...
        const result = await page.evaluate(PARSE_ATTACK_ATTRIBUTES_JS);
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  const data = await fetchAttackAttributes({
    delayMs: 3000,
